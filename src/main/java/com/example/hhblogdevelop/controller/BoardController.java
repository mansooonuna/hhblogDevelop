...
import com.example.hhblogdevelop.dto.BoardFileRequestDto;
import com.example.hhblogdevelop.dto.BoardFileResponseDto;
import com.example.hhblogdevelop.service.BoardService;
...
@Tag(name = "Board Controller", description = "파일 업로드 관련 api")
@RestController
@RequestMapping("/api")
...
public class BoardController {
    ...
    public void downloadFile(@RequestParam Long id, HttpServletResponse response) throws Exception {
        try {
            // 파일정보를 조회한다.
            BoardFileResponseDto fileInfo = boardService.findById(id);
    ...
    public String updateDeleteYn(Model model, BoardFileRequestDto boardFileRequestDto) throws Exception {
        try {
            model.addAttribute("result", boardService.updateDeleteYn(boardFileRequestDto.getIdArr()));
        } catch (Exception e) {
            throw new Exception(e.getMessage());
        }
